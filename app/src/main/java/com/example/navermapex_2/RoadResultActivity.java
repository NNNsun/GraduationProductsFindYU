...
public class RoadResultActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private double latitude = 35.831009; // 위도
    private double longitude = 128.756790; // 경도
    private int defaultZoom = 15; // 줌
    ...
    private String timeData, distanceData;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String src_address = iData.getStringExtra("src_address");
        String dst_userInput = iData.getStringExtra("dst_userInput");
        ...
        if (src_coordinate == null) { // 장소 검색->목적지 선택한 경우 출발지는 GPS로 현재위치 가져오기
            if(mainactivity.check_src == true) {
                src_search.setText(mainactivity.src_input); // 출발지 주소 셋팅
            ...
            if(mainactivity.check_src == true) {
                ...
                src_search.setText(src_address); // 출발지 주소 셋팅
            }
        ...
        if(isRoad) { // 길찾기 이용
            ...
            dst_search.setText(dst_userInput);
        }
        else { //길찾기 이용x
...
//        groundOverlay.setMap(naverMap);
        //회색배경 이미지추가
        GroundOverlay graybackground = new GroundOverlay();
        graybackground.setBounds(new LatLngBounds(
                new LatLng(34.822450, 127.751790), new LatLng(36.838070, 129.765200)));
        graybackground.setImage(
                OverlayImage.fromResource(R.drawable.gray));
        graybackground.setAlpha(1); //투명도
        graybackground.setMap(naverMap);

        //통합 이미지추가
        GroundOverlay groundOverlay = new GroundOverlay();
        groundOverlay.setBounds(new LatLngBounds(
                new LatLng(35.822450, 128.751790), new LatLng(35.838070, 128.765200)));
        groundOverlay.setImage(
                OverlayImage.fromResource(R.drawable.map_merge));
        groundOverlay.setAlpha(1); //투명도
        groundOverlay.setMap(naverMap);
        ...
        String serverpath = "local/test2.json";//필요없긴함
        JsonConverter jc = new JsonConverter(RoadResultActivity.this, serverpath);
...
//                path.setWidth(1);
        ...
        LatLng start = (new LatLng(startfindy - Integer.parseInt(mainactivity.start_y)*0.016300/div_ns, startfindx + Integer.parseInt(mainactivity.start_x)*0.018300/div_ew));
        Marker marker = new Marker();
        marker.setPosition(start);
        marker.setIcon(OverlayImage.fromResource(R.drawable.marker_start)); //출발마커
        marker.setWidth(113);
        marker.setHeight(150);
        marker.setMap(naverMap); // 좌표 상 장소를 마커로 표시

        LatLng end = (new LatLng(startfindy - Integer.parseInt(mainactivity.dest_y)*0.016300/div_ns, startfindx + Integer.parseInt(mainactivity.dest_x)*0.018300/div_ew));
        Marker marker_end = new Marker();
        marker_end.setPosition(end);
        marker_end.setIcon(OverlayImage.fromResource(R.drawable.marker_arrive)); //도착마커
        marker_end.setWidth(113);
        marker_end.setHeight(150);
        marker_end.setMap(naverMap); // 좌표 상 장소를 마커로 표시
        //서버에서 경로받아오는 시간만큼 딜레이붜야함
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                for(int r[] : route){
                    ...
                    if(count%3 == 0) { //2번마다 경로체킹
//                        System.out.println(r[0] + " " + r[1]);
                        findpoints.add(new LatLng(startfindy - r[0] * 0.016300 / div_ns, startfindx + r[1] * 0.018300 / div_ew));
                    }
                    count = count+1;
                }

//                findpoints.add(new LatLng(startfindy - 116 * 0.016300 / div_ns, startfindx + 331 * 0.018300 / div_ew));
//                findpoints.add(new LatLng(startfindy - 156 * 0.016300 / div_ns, startfindx + 381 * 0.018300 / div_ew));
//                findpoints.add(new LatLng(startfindy - 200 * 0.016300 / div_ns, startfindx + 453 * 0.018300 / div_ew));
//                findpoints.add(new LatLng(startfindy - 241 * 0.016300 / div_ns, startfindx + 463 * 0.018300 / div_ew));

                PathOverlay findpath = new PathOverlay();
                findpath.setCoords(findpoints);
                findpath.setPatternImage(OverlayImage.fromResource(R.drawable.pathpattern2));
                findpath.setColor(Color.parseColor("#1a59ff"));
                findpath.setPatternInterval(50);
                findpath.setOutlineWidth(0);
                findpath.setMap(naverMap);
                findpath.setWidth(20);
                findpath.setZIndex(2);

                //격자 1칸당 2m
                //평균 사람 걷는속도 3~5km/h 성인남성기준 평균 4km/h로 잡는다 -> 66.6/min -> 1.11/sec ->2m에 1.8초
                //count = 통과된 경로 수
                double pathdistance = 2*count;
//                System.out.println("count:"+count);
//                System.out.println("총 거리 : 약"+pathdistance/1000+"km");
                distanceData = String.valueOf(pathdistance/1000)+"km";
                distance.setText(distanceData);
                //시간 = count*1.8초
                double pathtime = count*1.8;
//                System.out.println("소요시간 : 약"+pathtime/60+"분");
                pathtime = Math.round(pathtime/60);
                timeData = String.valueOf((int)pathtime)+"분";
